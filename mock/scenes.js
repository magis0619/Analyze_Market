...
function makeMotes(count, spread, color, size, seed) {
  const r = rng(seed);
...
function driftMotes(pts, t, speed = 0.28) {
  const p = pts.geometry.attributes.position;
  const { phase, base, spread } = pts.userData;
  for (let i = 0; i < phase.length; i++) {
    const y = (base[i * 3 + 1] + t * speed) % spread.y;
    p.array[i * 3] = base[i * 3] + Math.sin(t * 0.6 + phase[i]) * 0.22;
    p.array[i * 3 + 1] = y;
    p.array[i * 3 + 2] = base[i * 3 + 2] + Math.cos(t * 0.45 + phase[i]) * 0.18;
  }
  p.needsUpdate = true;
}

function rock(w, h, d, color, seed) {
...
function glowSprite(color, size, intensity = 1) {
