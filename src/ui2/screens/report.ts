...
import {
  actionBar, button, elementLabel, figures, itemRow, toasts, topBar
} from '../components';
import { BEAT_ICON, BEAT_TONE, beatsOf } from '../expedition';
import { each, esc, num, when } from '../dom';

// 帰還レポート（docs/UI-SPEC.md §2.5 ＋ カード脱却指示書 §4）。
//
// **見どころ3行がこの画面で最も重要**（仕様書 §7.3）。
// なぜその結果になったかが分からないと、完全な運ゲーに感じられる。
// 到達深度は 3D 側（descent シーン）が光の帯で示すので、ここでは数字だけ扱う。
//
// **板を8枚積むのをやめ、1枚の巻物にする**（§4）。
// 見出し・見どころ・道中・数字・次の一手・戦利品——どれも「1回の報告」の
// 一部なのに、同じ角丸の板に切り分けて縦に積んでいた。境目の数だけ
// 「別の話が始まった」と読めてしまい、1本の物語として読めなかった。
// 3D は使わない（§4）。羊皮紙の地と封蝋の区切りは、色と形の話でしかない。
 ...
 * 巻物の区切り（カード脱却指示書 §4）。
 *
 * 板を切り替える代わりに、封蝋を押した罫で章を分ける。
 * **線だけにしない**——同じ太さの罫が並ぶと、結局それは表の行になる。
 * 印が1つ入るだけで「ここで話が変わる」が読めるようになる。
 */
function seal(label: string): string {
  return `<div class="seal"><i></i><span>${esc(label)}</span><i></i></div>`;
}
...
<div class="stack hero">
  ...
  <div class="scroll ${died ? 'ill' : ''}" data-role="scroll"
       style="animation-delay:${Math.max(-1.2, -t).toFixed(2)}s">
    <div class="scroll-in">

      <div data-role="headline" class="sc-head">
        <span class="en hd" style="color:var(--${tone})">${esc(r.headline.split('／')[0] ?? '')}</span>
        <span class="sub">${r.depth} / ${r.encountersTotal}</span>
      </div>
      ${when(r.headline.includes('／'),
        `<div class="sc-lead">${esc(r.headline.split('／').slice(1).join('／'))}</div>`)}

      ${seal('見どころ')}
      <div class="beats">${each(r.highlights, (h, i) =>
        `<div class="beat ${i === 0 ? 'key' : ''}"><i></i><span>${esc(h)}</span></div>`)}</div>

      ${when(beats.length > 2, `${seal('道中')}
      ...
      ${when(tips.length > 0, `${seal('次の一手')}
      <div class="beats">${each(tips, x =>
      ...
      ${when(r.loot.length > 0 || r.gold > 0, `
        <div class="sc-note" style="color:var(--down)">
          ${when(r.loot.length > 0, `未鑑定品 ${r.loot.length}個も失われた<br>`)}
          ${when(r.gold > 0, `持ち帰るはずだった ${num(r.gold)}G`)}
        </div>`)}
      <div class="sc-note">冒険者本人は無事に帰還した。最低限の装備は支給される。</div>`
        : `${seal('戦利品')}
      ...
      ${when(r.loot.length > 4, `<div class="sc-note">ほか ${r.loot.length - 4}個</div>`)}
